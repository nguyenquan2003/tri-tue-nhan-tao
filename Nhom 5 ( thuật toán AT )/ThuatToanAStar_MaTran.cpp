...
public:
	...
	int herStic() {
		...
		if (checker == 1) {
			target[0] = 1; target[1] = 2; target[2] = 3;
			target[3] = 8; target[4] = 0; target[5] = 4;
			target[6] = 7; target[7] = 6;
		}
		else {
			target[0] = 1; target[1] = 2; target[2] = 3;
			target[3] = 4; target[4] = 5; target[5] = 6;
			target[6] = 7; target[7] = 8;
		}

		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (arr[i][j] != target[i * 3 + j]) sum++;
			}
		}
...
bool checkFinish() {
	...
	if (puzzle[1][0] == 8) counter1++;
	if (puzzle[1][2] == 4) counter1++;
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if ((row < i && puzzle[i][j] < counter && puzzle[i][j] != 0) ||
					(row == i && col < j && puzzle[i][j] < counter && puzzle[i][j] != 0)) {
					sum++;
				}
			}
		}
